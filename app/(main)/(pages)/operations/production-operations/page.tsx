...
const ProductionOperationPage = () => {
  ...
  const actionBodyTemplate = (rowData: ProductionTrack, options: { rowIndex: number }) => {
    return (
      <>
        <Button size="small" type='button' icon="pi pi-list" className='mr-2' onClick={() => onShowOutput && onShowOutput(rowData)} severity="warning" />
      ...
      <form onSubmit={handleSubmit(submit)}>
        ...
        <div className="flex flex-align-items-center">
          ...
          <div className="flex flex-align-items-center mr-2">
            <div className="flex align-items-center gap-2">
              ...
              <FormMultiDropdown
                ...
                placeholder="Select" options={processOptions}
              />
              <Button disabled={!trackFilter.section_id} loading={loadings.fetchingOperator} severity="help" onClick={() => fetchTracks()} type='button' size='small' className="mt-2" icon="pi pi-refresh" label="Reload" />
            </div>
          </div>
          <div className="flex align-items-center gap-2 ml-auto">
            ...
            <FormCalendar disabled={!trackFilter.section_id} value={trackFilter.date} onChange={(e: any) => setTrackFilter({ ...trackFilter, date: e.value })} label="Operation Date" />
            <div className="mt-2">
              <Button size='small' disabled={!trackFilter.section_id} severity="help" onClick={onNextDateClick} icon="pi pi-arrow-right" />
            </div>
            <Button disabled={!trackFilter.section_id} loading={loadings.fetchingOperator} severity="help" onClick={addNewItem} type='button' size='small' className="mt-2" icon="pi pi-plus" label="Add Process" />
            <Button disabled={!trackFilter.section_id} loading={loadings.fetchingOperator || loadings.storingTracks} severity="success" size='small' type='submit' className="mt-2" icon="pi pi-save" label="Save" />
          </div>
        </div>
            ...
            <Column
              ...
              body={(_row: any, options: { rowIndex: number }) => (
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <FormInputText {...field} inputClassName='w-full' placeholder="Notes" errorMessage={fieldState.error?.message} isError={fieldState.error ? true : false} />
                  )}
                />
              )}
            />
